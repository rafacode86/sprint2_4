...
db.restaurants.find();
//2.Escriu una consulta per mostrar el restaurant_id, name, borough i cuisine de tots els documents en la col·lecció 
// Restaurants.
db.restaurants.find({},{restaurant_id : 1, name :  1, borough : 1, cuisine :1});
//3.Escriu una consulta per mostrar el restaurant_id, name, borough i cuisine, però excloent el camp _id per 
// tots els documents en la col·lecció Restaurants.
db.restaurants.find({},{restaurant_id : 1, name :  1, borough : 1, cuisine :1, _id :0});
//4.Escriu una consulta per mostrar restaurant_id, name, borough i zip code, però excloent el camp _id per tots els 
...
//13.Escriu una consulta per trobar els restaurants que no preparen menjar 'American ', tenen alguna 
// nota 'A' i no pertanyen a Brooklyn. S'ha de mostrar el document segons la cuisine en ordre descendent.
...
//no se puede usar /^.../ pero al final, hay que usar la funcion $regex
...
db.restaurants.find({"cuisine": { $in: ["American ", "Chinese"] },"borough": { $ne: "Brooklyn" }});
//18.Escriu una consulta per trobar el restaurant_id, name, borough i cuisine per aquells restaurants que 
...
//19.Escriu una consulta per trobar el restaurant_id, name, borough i cuisine per a aquells restaurants que 
// NO pertanyen a Staten Island, Queens, Bronx o Brooklyn.
db.restaurants.find({borough: {$nin:["Staten Island","Queens","Bronx","Brooklyn"]}},{_id:0,restaurant_id:1,name:1,borough:1,cuisine:1})
//20.Escriu una consulta per trobar el restaurant_id, name, borough i cuisine per a aquells restaurants que 
// aconsegueixin una nota menor que 10.
...
db.restaurants.find({ cuisine: "Seafood", cuisine: { $nin: ["American ", "Chinese"] }, name: { $not: /^Wil/i }}, { restaurant_id: 1, name: 1, borough: 1, cuisine: 1, _id: 0 })
...
// l'array de graus conté un grade de "A" i un score 9 amb un ISODate "2014-08-11T00:00:00Z".
...
// on el segon element de l'array coord conté un valor entre 42 i 52.
...
db.restaurants.find({}, { restaurant_id: 1, name: 1, borough: 1, cuisine: 1, _id: 0 }).sort({ name: 1 })
...
//29.Escriu una consulta que seleccioni tots els documents en la col·lecció de restaurants on els valors del camp coord és 
// de tipus Double.

//30.Escriu una consulta que seleccioni el restaurant_id, name i grade per a aquells restaurants que retornen 0 com a residu 
// després de dividir algun dels seus score per 7.
